...
- [x] **Step 1: 기존 테스트를 `lookup()` 대상으로 재작성 (failing)**
...
- [x] **Step 1: `test/llm.test.ts` 수정 (failing)**
...
- [x] **Step 3: `search()`의 fetch 블록을 `lookup()` 호출로 교체**
...
- [x] **Step 4: PROVIDERS에서 OpenAI 제거**
...
- [x] **Step 5: 자산 경로 `withBase()` 적용 + 안내 문구에서 OpenAI 제거**
...
- [x] **Step 3: `package.json` 스크립트/의존성 정리**
